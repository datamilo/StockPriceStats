"""
Generate a lightweight standalone HTML dashboard that loads data from parquet file.
This creates a much smaller HTML file (~50KB) that loads data dynamically.
"""
...
OUTPUT_FILE = SCRIPT_DIR / 'consecutive_breaks_dashboard_lite.html'
...
# Create HTML with data loading from parquet
...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/apache-arrow@15.0.0"></script>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="info-box">
            💡 <strong>Tip:</strong> This version loads data dynamically from the parquet file. Ensure the HTML file is in the same directory as price_data_filtered.parquet.
        </div>
    ...
    <script>
        ...
        async function loadData() {{
            try {{
                ...
                for (let row of data) {{
                    ...
                    allData[stock].dates.push(new Date(row.date).toISOString().split('T')[0]);
                    allData[stock].open.push(row.open);
                    allData[stock].high.push(row.high);
                    allData[stock].low.push(row.low);
                    allData[stock].close.push(row.close);
                }}

                // Sort each stock's data by date
                for (let stock of Object.keys(allData)) {{
                    const indices = allData[stock].dates.map((_, i) => i).sort((a, b) =>
                        allData[stock].dates[a].localeCompare(allData[stock].dates[b])
                    );

                    allData[stock] = {{
                        dates: indices.map(i => allData[stock].dates[i]),
                        open: indices.map(i => allData[stock].open[i]),
                        high: indices.map(i => allData[stock].high[i]),
                        low: indices.map(i => allData[stock].low[i]),
                        close: indices.map(i => allData[stock].close[i])
                    }};
                }}

                console.log(`Loaded ${{Object.keys(allData).length}} stocks`);
                ...
                document.getElementById('loadingIndicator').innerHTML = `
                    <div style="color: red;">
                        ...
                        <p>${{error.message}}</p>
                        <p>Make sure price_data_filtered.parquet is in the parent directory (../../)</p>
                    </div>
                `;
            }}
        }}
        ...
        function updateDisplay() {{
            const stockData = allData[currentStock];
            if (!stockData) return;
...
print(f"✅ Lightweight dashboard created successfully!")
print(f"📄 File: {OUTPUT_FILE}")
print(f"📊 Size: Much smaller (loads data dynamically)")
print(f"\n💡 This version requires price_data_filtered.parquet in the parent directory.")
print(f"   Place both files in the same folder to use the dashboard.")
