...
This script tests whether options that previously had high probability (80%+)
...
Note: All Days to Expiry (DTE) calculations use CALENDAR DAYS, not market/business days.
...
# Analysis parameters - MULTIPLE THRESHOLDS for flexible analysis
HISTORICAL_PEAK_THRESHOLDS = [0.80, 0.85, 0.90, 0.95]  # Test multiple peak thresholds
CURRENT_PROB_BINS = [(0.60, 0.70), (0.70, 0.80), (0.80, 0.90)]
DTE_BINS = [(0, 7), (8, 14), (15, 21), (22, 28), (29, 35), (36, 999)]

print("=" * 80)
print("FALLEN ANGELS ANALYSIS (MULTI-THRESHOLD)")
print("=" * 80)
print()
print(f"Testing historical peak thresholds: {[f'{t:.0%}' for t in HISTORICAL_PEAK_THRESHOLDS]}")
print("Comparing fallen angels vs baseline options at current probability levels")
print()
...
# =============================================================================
# IDENTIFY FALLEN ANGELS FOR EACH PROBABILITY METHOD AND THRESHOLD
# =============================================================================
print("Identifying 'fallen angel' options for all thresholds...")
...
for prob_col, prob_label in PROB_COLUMNS.items():
    ...
    # For each option, track if it ever hit each threshold
                ...
                if bin_min <= current_prob < bin_max:
                    ...
                    for dte_min, dte_max in DTE_BINS:
                        if dte_min <= dte <= dte_max:
                            # Create records for EACH threshold
                            for threshold in HISTORICAL_PEAK_THRESHOLDS:
                                is_fallen_angel = peak_prob >= threshold
    ...
    for prob_method in df_threshold['ProbMethod'].unique():
        ...
        for current_bin in df_method['CurrentProb_Bin'].unique():
            ...
            for dte_bin in df_bin['DTE_Bin'].unique():
                ...
                # Calculate for baseline (never hit threshold)
...
# Find best scenarios for EACH threshold
for threshold in sorted(df_summary['HistoricalPeakThreshold'].unique()):
    print(f"\n{'='*80}")
    print(f"THRESHOLD: {threshold:.0%}+ Historical Peak")
    print(f"{'='*80}")

    df_threshold = df_summary[df_summary['HistoricalPeakThreshold'] == threshold]
    best_results = df_threshold[df_threshold['Fallen_N'] >= 100].copy()  # Require 100+ samples

    if len(best_results) > 0:
        best_results = best_results.nlargest(5, 'Advantage_pp')

        print(f"\nTop 5 scenarios at {threshold:.0%}+ threshold:")
        print()
        ...
        print("  No results with sufficient sample size (100+ samples)")
...
print("2. Run generate_fallen_angels_report.py to create interactive HTML")
